  ...
  useEffect(() => {
    ...
    function init() {
        ...
        if (state.tasks.length === 0) {
            showToast("Đang tải kho chủ đề mặc định...", 'info');
            loadDefaultTopics();
        }
    }
    ...
    async function loadDefaultTopics() {
        const topics = [
            'IELTS_Accident', 'IELTS_Advertising', 'IELTS_Animal', 'IELTS_Appearance', 'IELTS_Art', 
            'IELTS_Celebrity', 'IELTS_City', 'IELTS_Countryside', 'IELTS_Covid', 'IELTS_Crime', 
            'IELTS_Culture', 'IELTS_Education', 'IELTS_Employment', 'IELTS_Energy', 'IELTS_Entertainment_Media', 
            'IELTS_Environment', 'IELTS_Family_Relationships', 'IELTS_Fashion', 'IELTS_Film', 'IELTS_Food', 
            'IELTS_Globalization', 'IELTS_Health', 'IELTS_History_Heritage', 'IELTS_Housing_Architecture', 
            'IELTS_Music', 'IELTS_Science', 'IELTS_Social_Issues', 'IELTS_Sports_Equipments', 'IELTS_Technology', 
            'IELTS_Tourism', 'IELTS_Transportation', 'IELTS_Weapons', 'IELTS_Weather'
        ];

        for (const topic of topics) {
            try {
                const res = await fetch(`/vocab/${topic}.csv`);
                const text = await res.text();
                const words = parseCSV(text);
                if (words.length > 0) {
                    addOrUpdateTask(topic.replace('IELTS_', '').replace('_', ' '), words, false);
                }
            } catch (e) {
                console.error("Failed to load topic:", topic);
            }
        }
        showToast("Đã tải xong 33 chủ đề IELTS!", 'success');
        renderTaskList();
    }
